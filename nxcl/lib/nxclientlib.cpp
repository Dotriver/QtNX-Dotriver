...
#ifdef NXCL_USE_NXSSH
    if (session->isHttpProxyEnabled()) {
        arguments.push_back ("-P");
        string host = session->getHttpProxyHost();
        unsigned short port = session->getHttpProxyPort();
        string username = session->getHttpProxyUsername();
        string password = session->getHttpProxyPassword();
        stringstream argProxy;
        ...
        argProxy << host << ":" << port;
        arguments.push_back(argProxy.str());
    }
#endif
...
#ifdef NXCL_USE_NXSSH
    string nxsshPath = NXUtility::getPath ("nxssh");
#else
    string nxsshPath = NXUtility::getPath ("ssh");
#endif
...
void NXClientLib::setSession (NXSession* nxSession)
{
    this->session = nxSession;
...
void NXClientLib::invokeProxy()
{
    ...
    if (sessionData->getGeometry() == "fullscreen") {
        stringstream resolution;

        ostringstream dimensionX, dimensionY;
        dimensionX << sessionData->getXRes();
        dimensionY << sessionData->getYRes();

        resolution << dimensionX.str() << "x" << dimensionY.str();
        res = resolution.str();
    } else {
        res = sessionData->getGeometry();
    }
    ...
    string openPath = NXUtility::getPath("open");
    ...
    data << "nx/nx" << x11Display;

    if (this->session->mediaEnabled()) {
        NXSound snd(&callbacks);
        int port = snd.startSoundServer(this);
        data << ",media=" << port;
    }
	
	if (this->session->microphoneEnabled()) {
		system("touch /tmp/charge");
        NXMicrophone micro(&callbacks);
        int port = micro.startMicrophoneStreaming(this);
        data << ",micro=" << port;
    }
 
    if (proxyData.encrypted) {
#ifdef NXCL_USE_NXSSH
        data << ",session=session,encryption=1,cookie="
            << proxyData.cookie
	...
	data << ",session=session,encryption=1,cookie="
        ...
        data << ",session=session,cookie=" << proxyData.cookie
    ...
    string nxproxyPath = NXUtility::getPath ("nxproxy");
...
void NXClientLib::startX11 (string resolution, string name)
{
    ...
    string nxwinPath = NXUtility::getPath("nxwin");
    ...
    string nxauthPath = NXUtility::getPath("nxauth");
