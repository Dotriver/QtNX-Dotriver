			...
			while (dbus_message_iter_next (&args)) {
				if (DBUS_TYPE_STRING == (t = dbus_message_iter_get_arg_type(&args))) {
					...
					case 3:
						...
						break;
					case 4:
						this->sessionData.setSessionName(ss.str());
						break;
					case 5:
						this->sessionData.setSessionType(ss.str());
						break;
					case 8:
						this->sessionData.setLinkType(ss.str());
						break;
					case 10:
						this->sessionData.setBackingstore(ss.str());
						break;
					case 13:
						this->sessionData.setGeometry(ss.str());
						break;
					case 14:
						this->sessionData.setKeyboard(ss.str());
						break;
					case 15:
						this->sessionData.setKbType(ss.str());
						break;
					case 17:
						this->sessionData.setAgentServer(ss.str());
						break;
					case 18:
						this->sessionData.setAgentUser(ss.str());
						break;
					case 19:
						this->sessionData.setAgentPass(ss.str());
						break;
					case 21:
						this->sessionData.setKey(ss.str());
						break;
					case 24:
						this->sessionData.setCustomCommand(ss.str());
						break;
					default:
					...
					case 1:
						...
						break;
					case 6:
						this->sessionData.setMemCache(iparam);
						break;
					case 7:
						this->sessionData.setImageCache(iparam);
						break;
					case 9:
						(iparam>0) ? this->sessionData.enableRender()
								: this->sessionData.disableRender();
						break;
					case 11:
						this->sessionData.setImageCompressionMethod(iparam);
						break;
					case 12:
						this->sessionData.setImageCompressionLevel(iparam);
						break;
					case 16:
						(iparam>0) ? this->sessionData.enableMedia()
								: this->sessionData.disableMedia();
						break;
					case 20:
						this->sessionData.setCupsPort(iparam);
						break;
					case 22:
						(iparam>0) ? this->sessionData.enableEncryption() 
								: this->sessionData.disableEncryption();
						break;
					case 23:
						(iparam>0) ? this->sessionData.enableFullscreen()
								: this->sessionData.disableFullscreen();
						break;
					case 25:
						(iparam>0) ? this->sessionData.enableVirtualDesktop()
								: this->sessionData.disableVirtualDesktop();
						break;
					default:
						this->callbacks.error ("ERROR: parameter type does not match its position in the message.");
						break;
...
{
	// Some defaults
	this->sessionData.setSessionName("default session");
	this->sessionData.setSessionType("unix-gnome");
	this->sessionData.setMemCache(8);
	this->sessionData.setImageCache(32);
	this->sessionData.setLinkType("adsl");
	this->sessionData.enableRender();
	this->sessionData.setBackingstore("when_requested");
	this->sessionData.setImageCompressionMethod(2);
	// this->sessionData.setImageCompressionLevel(0);
	this->sessionData.setGeometry("800x600+0+0"); // This'll be the size of the session
	this->sessionData.setKeyboard("defkeymap");
	this->sessionData.setKbType("pc102/defkeymap");
	this->sessionData.disableMedia();
	this->sessionData.setAgentServer("");
	this->sessionData.setAgentUser("");
	this->sessionData.setAgentPass("");
	this->sessionData.setCupsPort(0);
	this->sessionData.enableEncryption();
	this->sessionData.setSuspended(false);
	this->sessionData.enableFullscreen(); // If true, session.geometry
					  // is ignored
	this->sessionData.disableVirtualDesktop();
...
Nxcl::startTheNXConnection (void)
{
	...
	this->nxclientlib.setSession (&this->sessionData);
	...
	if (this->sessionData.getKey().size() == 0) { // Shouldn't need this->sessionData.encryption here.
		...
		this->nxclientlib.invokeNXSSH("supplied",
					      this->nxserver,
					      this->sessionData.encryptionEnabled(),
					      this->sessionData.getKey(), this->nxport);
	}
}
...
Nxcl::sendResumeList (list<NXResumeData>& resumable)
{
	...
	for (it=resumable.begin(); it!=resumable.end(); it++) {
		...
		const char* sessionType = (*it).getSessionType().c_str();
		const char* sessionID = (*it).getSessionId().c_str();
		const char* options = (*it).getOptions().c_str();
		const char* screen = (*it).getScreen().c_str();
		const char* available = (*it).getAvailable().c_str();
		const char* sessionName = (*it).getSessionName().c_str();
		int display = (*it).getDisplay();
